...
export class AddTicketComponent implements OnInit {
  ...
  ngOnInit() {
    var emp_data = JSON.parse(localStorage.currentUser);
    this.emp_number = emp_data.data.employee_number
    // temporary emp list
    // this.employee_data=[
    //   {name: 'Krish', id: 16790},
    ...
    this.ticket_service.getAllAdminsId()
      .subscribe(
        response => {
          // this.departments = response;
          // console.log("data",response);
          var all_admins = response['data'][0]['admins']
          var data = all_admins['FAST'].concat(all_admins['Internal IT-Application Support'], all_admins['IT-Infrastructure'], all_admins['Finance'], all_admins['Facilities'], all_admins['HR'], all_admins['Quality'], all_admins['Super Admin'], all_admins['GW_AMS'],);
          console.log("sddwew", data)
          if (data) {
            if (data.indexOf(this.emp_number) > -1) {
              console.log("insideif")
              this.onBehalfOf_bool = true
            } else {
              console.log("insideelse")
              this.onBehalfOf_bool = false
  ...
  get_employee_list() {
    this.ticket_service.getAllEmployeeDetails()
      .subscribe(
        response => {
          console.log("data", response['data']['employees'])
          this.over_all_emp = response['data']['employees']
          this.employee_data = response['data']['employees']
          this.behalf_of_name = this.employee_data;
  ...
  add_ticket() {
    if (this.row_form_data.status == 'VALID') {
      ...
      delete this.row_form_data.value.onBehalfOf_search
      console.log("asdasd", this.row_form_data.value)
  ...
  update_competency() {
    ...
    this.behalf_of_name = this.employee_data.filter(competency => {
      return competency.itemName.toLowerCase().includes(filterValue)
    })
  }
